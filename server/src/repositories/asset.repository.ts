...
interface AssetStatsOptions {
  ...
  /**
   * #869 follow-up: server-derived from `auth`, never from the DTO. See the matching field on
   * `AssetBuilderOptions`.
   */
...
interface AssetBuilderOptions {
  ...
  visibility?: AssetVisibility;
  /**
   * #869 follow-up: whether the session has entered the Locked Folder PIN. Server-derived from
   * `auth` by timeline.service, NEVER read off the DTO — `visibility` is client-settable and
   * `visibility=hidden` is what the motion half of a locked live photo carries, so this is what lets
   * the builder tell "show me my hidden assets" apart from "show me the locked pair I can't open".
...
export function withTimeBucketAssetFilters<O>(
  ...
  return (
    qb
      ...
      // #869 follow-up: an explicit `visibility=hidden` bypasses withDefaultVisibility and matches the
      // motion half of a locked live photo, which keeps `hidden` when its still is locked — so a session
      // that never entered the PIN could enumerate a locked live photo's id, capture date, duration and
      // city through the timeline. Only Hidden needs the gate: Timeline/Archive never match a motion row,
      // the undefined case is Archive+Timeline, and Locked already requires elevation in timeline.service.
  ...
  getStatistics(
    ...
    return (
      this.db
        ...
        // #869 follow-up: same gate as withTimeBucketAssetFilters — an explicit `visibility=hidden`
        // otherwise counts the motion half of a locked live photo into the stats a non-elevated session
        // is shown.
        .$if(visibility === AssetVisibility.Hidden && !hasElevatedPermission, (qb) =>
          qb.where((eb) => isNotLockedAsset(eb)),
        )
