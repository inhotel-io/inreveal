...
const adjust = (level: TonalLevel) =>
  ({ action: AssetEditAction.Adjust, parameters: { brightness: level } }) as const;
...
describe('mergeEdits', () => {
  ...
  it('incoming crop replaces existing crop and stays first', () => {
    const merged = mergeEdits([crop, adjust(TonalLevel.SlightIncrease)], [
