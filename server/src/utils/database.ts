 ...
 * which switches off the owner/space scoping in searchAssetBuilder. Re-gate to
 * match the album grid's own flat visibility gate (withDefaultVisibility) —
 * AlbumRead (checked upstream of this call) already authorizes the album's
 * content, so no per-asset space-path check is needed here.
 *
 * L2 (over-restriction fix): an earlier version of this gate additionally
 * anti-joined out any asset that ALSO happened to be directly-space-linked or
 * library-linked (shared_space_asset / shared_space_library), re-admitting them
 * only via a `timelineSpaceIds`-scoped arm — which required the caller to both
 * be a member of the LINKING space and have that member's showInTimeline toggle
 * on. That is stricter than the album grid (inAlbums + withDefaultVisibility,
 * src/utils/database.ts inAlbums), which never anti-joins on shared_space_*: an
 * album_user Viewer with no space membership at all, or a space member with the
 * timeline toggle off, could not see an otherwise-visible library-backed /
 * space-linked album asset via search even though the grid showed it. Flattened
 * to match the grid exactly: Archive+Timeline, not-deleted, no owner exception,
 * no anti-join, no space-path check.
 *
 * `timelineSpaceIds` is kept in the signature — searchAssetBuilder's single call
 * site (search) still computes and passes it for the sibling spaceId/
 * timelineSpaceIds branches above — but is no longer used by this gate.
 */
