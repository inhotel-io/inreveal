...
import { ResolvedImagePreset, resolveImagePreset, selectDerivedImageSource } from 'src/utils/image-preset';
...
export class AssetMediaService extends BaseService {
  ...
  private async renderDerivedImage(
    ...
    const tempPath = StorageCore.getTempPathInDir(
      StorageCore.getNestedFolder(StorageFolder.Thumbnails, asset.ownerId, filename),
    );
