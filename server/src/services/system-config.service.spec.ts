...
import { ReleaseChannel, SystemConfigSchema } from 'src/dtos/system-config.dto';
import {
  AudioCodec,
  Colorspace,
  CQMode,
...
describe(SystemConfigService.name, () => {
  ...
  describe('getConfig', () => {
    ...
    it('should default image.presets to empty so a stock install renders nothing extra', async () => {
      ...
      await expect(sut.getSystemConfig()).resolves.toMatchObject({ image: { presets: {} } });
    });
    ...
    it('should reject a derived image preset with a malformed aspect ratio', async () => {
      ...
      await expect(sut.getSystemConfig()).rejects.toThrow('[image.presets.hero.aspectRatio] Aspect ratio must be W:H');
    });
    ...
    it('should reject a derived image preset with a zero side, duplicate widths or an empty width list', async () => {
      ...
      await expect(sut.getSystemConfig()).rejects.toThrow('[image.presets.hero.widths] Widths must be unique');

      mocks.systemMetadata.readFile.mockResolvedValue(
        JSON.stringify({ image: { presets: { hero: { aspectRatio: '16:9', widths: [] } } } }),
      );
      await expect(sut.getSystemConfig()).rejects.toThrow('[image.presets.hero.widths]');
    });
