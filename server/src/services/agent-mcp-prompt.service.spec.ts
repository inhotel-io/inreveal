...
describe(AgentMcpPromptService.name, () => {
  ...
  it('keeps the committed runner prompt module in sync with the renderer', async () => {
    ...
    expect(committedModule).toContain('Generated by server/src/bin/sync-agent-mcp-prompt.ts');
    expect(committedModule).toContain('export const galleryMcpPromptCheatSheet =');
  });
});
