...
class MergedAssetDrift extends i1.ModularAccessor {
  ...
  i0.Selectable<MergedBucketResult> mergedBucket({
    ...
    $arrayStartIndex += userIds.length;
    return customSelect(
      'SELECT COUNT(*) AS asset_count, bucket_date FROM (SELECT CASE WHEN ?1 = 0 THEN COALESCE(STRFTIME(\'%Y-%m-%d\', rae.local_date_time), STRFTIME(\'%Y-%m-%d\', rae.created_at, \'localtime\')) WHEN ?1 = 1 THEN COALESCE(STRFTIME(\'%Y-%m\', rae.local_date_time), STRFTIME(\'%Y-%m\', rae.created_at, \'localtime\')) WHEN ?1 = 4 THEN COALESCE(STRFTIME(\'%Y\', rae.local_date_time), STRFTIME(\'%Y\', rae.created_at, \'localtime\')) END AS bucket_date FROM remote_asset_entity AS rae LEFT JOIN stack_entity AS se ON rae.stack_id = se.id WHERE rae.deleted_at IS NULL AND rae.visibility = 0 AND(rae.owner_id IN ($expandeduserIds) OR EXISTS (SELECT 1 AS _c0 FROM shared_space_asset_entity AS ssa INNER JOIN shared_space_member_entity AS ssm ON ssm.space_id = ssa.space_id WHERE ssa.asset_id = rae.id AND ssm.user_id = ?2 AND ssm.show_in_timeline = 1) OR EXISTS (SELECT 1 AS _c1 FROM shared_space_library_entity AS ssl INNER JOIN shared_space_member_entity AS ssm ON ssm.space_id = ssl.space_id WHERE ssl.library_id = rae.library_id AND ssm.user_id = ?2 AND ssm.show_in_timeline = 1) OR EXISTS (SELECT 1 AS _c2 FROM shared_space_album_asset_entity AS ssaa INNER JOIN shared_space_album_link_entity AS ssal ON ssal.album_id = ssaa.album_id INNER JOIN shared_space_member_entity AS ssm ON ssm.space_id = ssal.space_id WHERE ssaa.asset_id = rae.id AND ssal.show_in_timeline = 1 AND ssm.user_id = ?2 AND ssm.show_in_timeline = 1))AND(rae.stack_id IS NULL OR rae.id = se.primary_asset_id)UNION ALL SELECT CASE WHEN ?1 = 0 THEN STRFTIME(\'%Y-%m-%d\', lae.created_at, \'localtime\') WHEN ?1 = 1 THEN STRFTIME(\'%Y-%m\', lae.created_at, \'localtime\') WHEN ?1 = 4 THEN STRFTIME(\'%Y\', lae.created_at, \'localtime\') END AS bucket_date FROM local_asset_entity AS lae WHERE NOT EXISTS (SELECT 1 FROM remote_asset_entity AS rae WHERE rae.checksum = lae.checksum AND rae.owner_id IN ($expandeduserIds)) AND EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 0) AND NOT EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 2)) GROUP BY bucket_date ORDER BY bucket_date DESC',
      variables: [
  ...
  i9.$SharedSpaceAlbumAssetEntityTable get sharedSpaceAlbumAssetEntity =>
      i1.ReadDatabaseContainer(
        attachedDatabase,
      ).resultSet<i9.$SharedSpaceAlbumAssetEntityTable>(
        'shared_space_album_asset_entity',
      );
  i10.$SharedSpaceAlbumLinkEntityTable get sharedSpaceAlbumLinkEntity =>
      i1.ReadDatabaseContainer(
        attachedDatabase,
      ).resultSet<i10.$SharedSpaceAlbumLinkEntityTable>(
        'shared_space_album_link_entity',
      );
  i11.$LocalAlbumAssetEntityTable get localAlbumAssetEntity =>
      i1.ReadDatabaseContainer(
        attachedDatabase,
      ).resultSet<i11.$LocalAlbumAssetEntityTable>('local_album_asset_entity');
  i12.$LocalAlbumEntityTable get localAlbumEntity => i1.ReadDatabaseContainer(
    attachedDatabase,
  ).resultSet<i12.$LocalAlbumEntityTable>('local_album_entity');
}
