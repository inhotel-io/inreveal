...
void main() {
  ...
  group('gallery-fork: from23To24 data migration', () {
    test('preserves existing v23 data and materializes new v24 objects', () async {
      ...
      await verifier.migrateAndValidate(dbForMigration, 24);
      await dbForMigration.close();

      final migratedDb = v24.DatabaseAtV24(schema.newConnection());
      ...
      final libraryCount = await migratedDb
          .customSelect('SELECT COUNT(*) AS c FROM library_entity')
          .getSingle();
      expect(libraryCount.data['c'], 0);
      final sharedSpaceLibraryCount = await migratedDb
          .customSelect('SELECT COUNT(*) AS c FROM shared_space_library_entity')
          .getSingle();
      expect(sharedSpaceLibraryCount.data['c'], 0);
      ...
      ''');
      final libCount = await migratedDb
          .customSelect(
            "SELECT COUNT(*) AS c FROM library_entity WHERE id = 'post-upgrade-lib'",
          )
          .getSingle();
      expect(libCount.data['c'], 1);
