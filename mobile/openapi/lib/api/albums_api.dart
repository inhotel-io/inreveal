...
class AlbumsApi {
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getAlbumNamesWithHttpInfo({ Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// Returns a lightweight list of albums available to the authenticated user for quick palette lookups.
  Future<List<AlbumNameDto>?> getAlbumNames({ Future<void>? abortTrigger, }) async {
    final response = await getAlbumNamesWithHttpInfo(abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
