...
class SharedSpacesApi {
  ...
  ///
  /// Reassign the selected assets' faces from this person to another person, or to a new person.
  ///
  /// Note: This method returns the HTTP [Response].
  ///
  /// Parameters:
  ...
  /// * [String] personId (required):
  ///
  /// * [SharedSpacePersonReassignDto] sharedSpacePersonReassignDto (required):
  Future<Response> reassignSpacePersonFacesWithHttpInfo(String id, String personId, SharedSpacePersonReassignDto sharedSpacePersonReassignDto, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final apiPath = r'/shared-spaces/{id}/people/{personId}/reassign'
      .replaceAll('{id}', id)
      .replaceAll('{personId}', personId);

    // ignore: prefer_final_locals
    Object? postBody = sharedSpacePersonReassignDto;
  ...
  ///
  /// * [SharedSpacePersonReassignDto] sharedSpacePersonReassignDto (required):
  Future<SharedSpacePersonReassignResponseDto?> reassignSpacePersonFaces(String id, String personId, SharedSpacePersonReassignDto sharedSpacePersonReassignDto, { Future<void>? abortTrigger, }) async {
    final response = await reassignSpacePersonFacesWithHttpInfo(id, personId, sharedSpacePersonReassignDto, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'SharedSpacePersonReassignResponseDto',) as SharedSpacePersonReassignResponseDto;
