...
- Smart-search facet totals and buckets use the same asset date field as current smart-search result filtering, `fileCreatedAt`. The response bucket shape remains the same as `getTimeBuckets` (`{ timeBucket, count }`), but it intentionally follows smart-search result semantics rather than media-date `localDateTime`.
