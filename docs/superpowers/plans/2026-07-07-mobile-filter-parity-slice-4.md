...
- **Placeholder scan:** "read the mirror" notes point to concrete files (when_picker._, person_picker._, \_CityCascade) for exact idioms — not TODOs; the novel logic (cap, single-select, lazy-on-expand, query filter) has concrete code/specs.
