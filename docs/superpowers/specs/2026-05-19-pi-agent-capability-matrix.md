...
- `searchAssets`: metadata search by date, location labels, camera fields,
  favorite state, album membership, tags, rating, media type, and bounded limit.
...
| Add/remove photos in space | “Add these vacation photos to the Family space.” | Needs new tool for name resolution | Search/read candidates, then propose `space.addAssets` or `space.removeAssets` if the assistant already has the space id. | Makes it clear this changes space membership, not the original library asset. | Existing space id supplied; no assets selected; already-in-space assets; remove without deleting; missing space lookup is handled clearly. |
| Update space details | “Rename Family to Family 2026.” | Needs new tool for name resolution | Propose `space.updateDetails` if the assistant already has the space id. | Shows changed fields and leaves assets/members untouched. | Space id supplied; color/description update; missing space lookup is handled clearly. |
...
| --- | --- | --- |
| People-based organization | Search does not expose person/person-id filters to Pi. | `listPeople`, `searchAssets` person filters, or `readPersonAssets`. |
| Natural-language semantic search | No semantic text-to-image search tool is exposed. | `semanticSearchAssets(query, filters, limit)` backed by existing or future embedding index. |
...
| Space lookup by name | Planning can target spaces by id, but the read tools do not expose list/read space helpers to the assistant. | `listSpaces`, `readSpace`, or space filters in existing search/read tools. |
| Large-library pagination | Current search is bounded, but no explicit cursor/chunk contract. | Cursor-based search/read APIs and plan chunking support. |
...
| Member management for spaces | Space plans do not add/remove members or change roles. | `space.addMembers`, `space.removeMembers`, `space.updateMemberRole` operations. |
...
6. “Archive old screenshots from 2024.”
7. “Tag my Berlin photos as Travel.”
8. “Rotate these sideways photos clockwise.”
9. “What will this plan change?”
10. “Actually exclude videos and show me the revised plan.”
...
3. Decide whether the next capability expansion should be people search,
   semantic search, or large-library pagination. People search is likely the
