...
Ownership Matrix. Sharing ships via `share_assets` / `shareLink.create`; it is
High risk, OUTWARD-FACING, and the `createSharedLinks` scope is granted only in
the LocalPowerUser preset (off in Careful/VisualOrganizer). The eval preset
keeps it off, so no link is created in any test or eval; live links are always
reviewable plans the user approves first.)
