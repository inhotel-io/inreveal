...
- Render tiles immediately from counts. Call `getTimeBuckets` without covers. **Initial tile state is a neutral skeleton, not a thumbhash blur** — the thumbhash now arrives *with* the cover from the new endpoint (it can't be in the counts response without paying for representative selection). The win is that the skeleton clears in a fast, cached, index-served cover fetch for the *visible* buckets, instead of waiting several seconds on a full sort of *all* buckets.
...
**Order: write/adjust the failing test first (RED), then implement (GREEN).** Several existing tests encode the *old* "representatives come from `getTimeBuckets`" behaviour and must be flipped first — they are the RED step, not collateral.
