...
`wrap-break-words` fixes this at the source: the token wraps, the overflow becomes vertical, and height-based detection sees it. The two are coupled — the class is what makes height-only measurement sufficient, so neither may be removed without the other.

Scenario R14 pins only the class's presence on the label. The resulting behaviour — that `wrap-break-words` actually changes what a real browser measures — is **not** provable by a unit test: happy-dom implements no layout engine, so `scrollHeight`/`clientHeight` are test-supplied numbers regardless of what classes are present (R12, which stubs those numbers directly, would pass unchanged even if `wrap-break-words` were deleted from the component). The proof is a real-browser probe instead, measured against this row's true geometry (200px wide, font-size 0.875rem, line-height 1.25rem):
...
| 120-char unbreakable token, **with** `wrap-break-words`    |            120 |             40 | Yes — tooltip                                 |
| 120-char unbreakable token, **without** `wrap-break-words` |             20 |             20 | **No** — false "fits"; the load-bearing proof |

The last row is the evidence that `wrap-break-words` is load-bearing rather than cosmetic: removing it collapses the unbreakable-token case from "overflowing, tooltip shown" to "not overflowing, false fits," with no automated test able to catch the regression.
...
| R12    | tooltip on overflow, regardless of name shape  | Given the height stub forces an overflow verdict (`stubHeights(100, 40)`) for a 120-char unbreakable-token name, then the tooltip is present — pins that an overflowing row always gets a tooltip. It does **not** prove `wrap-break-words` causes that overflow in the first place (the name is inert here); see the Hazard section above for the real-browser proof of that.                                                                                                                                                                                                                                                                                                       |
| R13    | dimmed variant                                 | Given `dimmed` is true, then the row element carries `opacity-50` and `font-medium`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| R14    | label allows mid-word breaks                   | Given any row, then the label carries `wrap-break-words` — pins only the class's presence, not the resulting overflow behaviour (see the Hazard section above).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
`scrollHeight > clientHeight` against `-webkit-line-clamp` is the standard detection technique, but happy-dom implements no layout, so **no automated test proves a real browser reports overflow for a clamped element**. The measurements backing items 1, 3, and 4 below (natural 1/2/3-line text, and the unbreakable-token with/without `wrap-break-words`) are recorded in the "Hazard: an unbreakable token defeats height-based detection" section above. Manual verification, recorded rather than assumed:
