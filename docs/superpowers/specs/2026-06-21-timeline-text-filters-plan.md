...
- [ ] **SQL impl:** in `withTimeBucketAssetFilters` (`asset.repository.ts:245`) add `|| !!options.description` to the existing `asset_exif`-join guard + the `description` `ilike` inside that block; add top-level `$if` blocks for `originalFileName` (no join) and `ocr` (`ocr_search` join + `%>>`, `tokenizeForSearch` already imported). Green.
- [ ] **Service passthrough test:** `timeline.service.spec.ts` — `getTimeBuckets`/`getTimeBucket`/`getTimeBucketCovers` forward the three fields into repository options (spread). No `timeline.service.ts` change.
- [ ] **Index migration:** add `migrations-gallery/<round-ts>-AddAssetExifDescriptionTrigramIndex.ts` — `up`: `CREATE INDEX IF NOT EXISTS "idx_asset_exif_description_trigram" ON "asset_exif" USING gin (f_unaccent("description") gin_trgm_ops);` `down`: drop it. (Mirrors the existing `originalFileName`/`ocr_search` trigram indexes.)
- [ ] **Medium DB test** (`pnpm test:medium`, requires the rolling DB) — seed assets with known description/filename/OCR; assert `getTimeBuckets` returns exactly the matching ids per field and combined; accent-insensitive (`café`⇄`cafe`); substring; OCR-on-empty-`ocr_search` → empty, not error.
