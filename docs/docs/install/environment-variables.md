...
| `IMMICH_MACHINE_LEARNING_CLOSE_CONNECTIONS`                 | When `true`, the server sends `Connection: close` on machine learning `/ping` and `/predict` requests. This can improve distribution behind connection-level load balancers, at the cost of disabling HTTP connection reuse for machine learning calls. |             `false`             | server           |
