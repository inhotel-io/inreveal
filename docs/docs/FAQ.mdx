...
### How can I see a list of all users in Gallery?
...
On iOS (iPhone and iPad), the operating system determines if a particular app can invoke background tasks based on multiple factors, most of which the Gallery app has no control over. To increase the likelihood that the background backup task is run, follow the steps below:

- Enable Background App Refresh for Gallery in the iOS settings at `Settings > General > Background App Refresh`.
- Disable **Low Power Mode** when not needed, as this can prevent apps from running in the background.
- Disable Background App Refresh for apps that don't need background tasks to run. This will reduce the competition for background task invocation for Gallery.
- Use the Gallery app more often.
...
### Does Gallery change the file?

No, Gallery does not modify the original files.
All edited metadata is saved in companion `.xmp` sidecar files and the database.
However, Gallery will delete original files that have been trashed when the trash is emptied in the Gallery UI.

### Why do my file names appear as a random string in the file manager?

When Storage Template is off (default) Gallery saves the file names in a random string (also known as random UUIDs) to prevent duplicate file names.
...
### Why are only photos and not videos being uploaded to Gallery?

This often happens when using a reverse proxy in front of Gallery.
...
### How can I backup data from Gallery?

See [Backup and Restore](/administration/backup-and-restore.md).

### Does Gallery support reading existing face tag metadata?
...
### Does Gallery support the filtering of NSFW images?
...
Files sent on WhatsApp are saved without metadata on the file. Therefore, Gallery has no way of knowing the original date of the file when files are uploaded from WhatsApp,
...
### Why do HDR videos appear pale in Gallery player but look normal after download?

Gallery uses a player with known HDR color display issues. We are experimenting with a different player that provides better color profiles for HDR content for future improvements.

### Why does Gallery transcode my videos to a lower quality?

Gallery always keeps your original files. Alongside that, it generates a transcoded version for compatibility and performance reasons.
...
### Can I keep my existing album structure while importing assets into Gallery?

Yes, by using the [Gallery CLI](/features/command-line-interface) along with the `--album` flag.
...
Gallery will attempt to delete original files that have been trashed when the trash is emptied.
In read-write external libraries (the default), Gallery will delete the original file.
In read-only libraries (`:ro` in the `docker-compose.yml`), files can still be trashed in the UI.
However, when the trash is emptied, the files will re-appear in the main timeline since Gallery is unable to delete the original file.
...
### Does Gallery support Facial Recognition for videos?

Gallery's machine learning feature operates on the generated thumbnail. If a face is visible in the video's thumbnail it will be picked up by facial recognition.
Scanning the entire video for faces may be implemented in the future.

### Does Gallery have animal recognition?
...
### Why is Gallery slow on low-memory systems like the Raspberry Pi?

Gallery optionally uses transcoding and machine learning for several features. However, it can be too heavy to run on a Raspberry Pi. You can [mitigate](/FAQ#can-i-lower-cpu-and-ram-usage) this or host Gallery's machine-learning container on a [more powerful system](/guides/remote-machine-learning), or [disable](/FAQ#how-can-i-disable-machine-learning) machine learning entirely.
...
### How can I see Gallery logs?

Gallery components are typically deployed using docker. To see logs for deployed docker containers, you can use the [Docker CLI](https://docs.docker.com/engine/reference/commandline/cli/), specifically the `docker logs` command. For examples, see [Docker Help](/guides/docker-help.md).
...
### How can I run Gallery as a non-root user?
...
### How can I purge data from Gallery?

Data for Gallery comes in two forms:
...
```bash title="Remove Gallery (containers and volumes)"
docker compose down -v
```

After removing the containers and volumes, there are a few directories that need to be deleted to reset Gallery to a new installation. Once they are deleted, Gallery can be started back up and will be a fresh installation.

- `DB_DATA_LOCATION` contains the database, media info, and settings.
- `UPLOAD_LOCATION` contains all the media uploaded to Gallery.

:::note Portainer
If you use portainer, bring down the stack in portainer. Go into the volumes section  
and remove all the volumes related to gallery then restart the stack.
:::
...
If it mentions SIGILL (note the lack of a K) or error code 132, it most likely means your server's CPU is incompatible with Gallery.
