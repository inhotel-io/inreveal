...
- **The memory lane is the #997 sibling: server-sourced, not local.** Memories are generated per owner (`MemoryService.createOnThisDayMemories` → `AssetRepository.getByDayOfYear([ownerId])`) and the `memory` / `memory_asset` sync streams are hard-filtered to `ownerId = userId` (`sync.repository.ts` `MemorySync` / `MemoryToAssetSync`), so the local Drift `memory_entity` can never hold a memory built from Space-shared photos. Web sees them because `GET /memories` reads the fork-only `MemoryRepository.searchAccessible`, then `MemoryService.search` RBAC-filters each memory's assets and drops the ones left empty. `DriftMemoryService.getMemoryLane` therefore reads that endpoint via `MemoryApiRepository.getMemoryLane` and falls back to the owner-scoped local `DriftMemoryRepository.getAll` only on server/offline failure. Two traps: (1) **do not pass `for`** — the endpoint validates it as `YYYY-MM-DD` (`isoDateToDate`) while the generated Dart client serialises every `DateTime` query param as a full ISO timestamp, so it 400s; without `for` the server applies `showAt` but not `hideAt`, which is why the show/hide window is re-applied client-side to mirror the local query. (2) `GET /memories/{id}` (`Permission.MemoryRead`) is still **owner-only** server-side, so `DriftMemoryService.get` — the by-id memory deep link — stays local/owner-scoped; web has the same limitation (its viewer resolves memories out of `searchMemories`, not `getMemory`).
